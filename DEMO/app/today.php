    ...
    <ul class="checklist">
      <?php foreach ($foods as $food): ?>
        <li class="checklist-item">
          <input type="checkbox" id="<?php $food['id']; ?>" name="foods[]" value="<?php echo $food['name']; ?>">
          <label for="<?php $food['id']; ?>"><?php echo $food['name']; ?></label>
        </li>
      <?php endforeach; ?>
    </ul>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $acneRating = $_POST['likert']; // Assuming the name attribute of the likert scale is "likert"
  $hairlossRating = $_POST['likert2']; // Assuming the name attribute of the likert scale is "likert2"
